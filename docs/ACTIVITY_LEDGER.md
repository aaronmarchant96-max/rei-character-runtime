...
| 2026-08-24 | Gate Qwen3 4B Instruct 2507 against the current live route | EXP-020 four-case candidate/control receipts | A stronger small model improves fact use but cannot erase CPU latency or clause-level embellishment | Separate immediate player interaction from slower character-material preparation | rejected as live replacement: 39.90 s warm grounded turn; production routing unchanged |
| 2026-08-24 | Add the first two-speed prepared character path | 53 tests; EXP-021 local/live receipts; human-confirmed screenshot | Exact approved material can bypass live generation without bypassing identity, fact-key, memory, voice, or evidence boundaries | Stronger models can prepare reusable character capability outside the player's wait path | accepted for one live Nels daughter turn: 0.174 ms dialogue, zero model calls/tokens/cost, 6.405 s Piper playback |
| 2026-08-26 | Turn accepted play into replayable C-Activity evidence | 66 tests; EXP-027; one accepted fixture; eight rejected regression mutations | Every new spoken turn is classified, while only named human acceptance can promote an interaction into the replay corpus | Real play can become reusable evidence for grounding, routing, voice-selection, latency-path, and authority changes | one Nels family fixture; no subjective quality, audio-quality, or general-learning claim |
| 2026-08-26 | Add the human review boundary to the C-Activity flywheel | 70 tests; EXP-028; live queue smoke test over 10 recorded turns | Content-addressed accepted/rejected decisions can be recorded without copying dialogue, while skips remain pending | Gameplay evidence can be triaged into local replay candidates and structured failure cohorts | local review only; no automatic fixture publication or policy change |
| 2026-08-26 | Aggregate the C-Activity evidence corpus | 73 tests; EXP-029; JSON/Markdown report over 10 unique interactions | Human judgments, automatic signals, integrity exceptions, cohorts, and measured latency distributions remain explicitly separated | Review activity can reveal recurring failure classes before any policy proposal is considered | 0/10 reviewed; historical corpus only; report has no policy authority |
