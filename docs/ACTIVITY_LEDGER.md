...
| 2026-08-24 | Add the first two-speed prepared character path | 53 tests; EXP-021 literal local receipt | Exact approved material can bypass live generation without bypassing identity, fact-key, memory, voice, or evidence boundaries | Stronger models can prepare reusable character capability outside the player's wait path | 2.496798 ms local wall time; zero live model calls/tokens/cost; in-game acceptance pending |
